-- Fix the race condition in bill number generation
...
BEGIN
  ...
  LOOP
    ...
    INTO bill_count
    FROM public.bills
    WHERE bill_number LIKE 'AL-' || current_year || '-%'
    AND bill_number ~ '^AL-[0-9]{4}-[0-9]+$';
    
    -- Generate the bill number
    bill_number := 'AL-' || current_year || '-' || LPAD(bill_count::TEXT, 4, '0');
    
    -- Check if this bill number already exists
    IF NOT EXISTS (SELECT 1 FROM public.bills WHERE bill_number = bill_number) THEN
      RETURN bill_number;
    END IF;
